...
BOOST_FUSION_ADAPT_STRUCT(glm::dvec3, x,y,z)
...
/*************replaced memory mapping by iostream due to a failed build on windows***************/
...
void obj_parser::extractProjectionRect
    (const std::string& _dir, boost::array<vert, ZAY_RECTANGLE_P>& points){
    ...
    auto position = x3::rule<struct _position, vert> {"position"}
                = "v" >> x3::double_ >> x3::double_ >> x3::double_;

    auto  rect = x3::skip(x3::blank | '#' >> *(x3::char_ - x3::eol) >> (x3::eol|x3::eoi)) 
                [ *x3::eol >> +(position >> (x3::eoi|+x3::eol)) ];

    std::vector<vert> primitives;
    if (x3::parse(_head, _tail, rect, primitives)){

        if(ZAY_RECTANGLE_P!=primitives.size()){
            ROS_ERROR_STREAM("corrupted file read <" << _dir << ">");
            exit(EXIT_FAILURE);
        }

        for(uint32_t i{0}; i<ZAY_RECTANGLE_P; ++i)
            points[i]=primitives[i];
               ...
               bool hMir, bool vMir)
{
    ...
    std::string _dir{ros::package::getPath(ZAY_PACKAGE_NAME)+dir};
